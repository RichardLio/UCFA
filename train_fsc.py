...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    model_an = Model(cfg, ch=3, nc=10, anchors=hyp.get('anchors')).to(device)  # create
    # model_an = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    if pretrained:
        # with torch_distributed_zero_first(LOCAL_RANK):
    ...
    modelan_statedict = model_an.state_dict()
    new_dict = {}
    for k in freeze:
        for k2, v in csd.items():
            if k in k2:
                new_dict.update({k2: v})
    ...
    modelan_statedict.update(new_dict)
    model_an.load_state_dict(modelan_statedict,strict=False)
    model = model_an
    nc = model.yaml['nc']
    # Image size
    ...
    # Trainloader
    train_path ="/mnt/ecc5c6c9-7631-4983-9ba0-1ec98729589b/Dataset/vehicle1"
    ...
    hyp['cls'] *= 1.0 * 3 / nl # scale to classes and layers
...
def parse_opt(known=False):
    ...
    parser.add_argument('--batch-size', type=int, default=42, help='total batch size for all GPUs, -1 for autobatch')
