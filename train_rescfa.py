...
def train(hyp, opt, device): 
    ...
    if pretrained:
        ...
        model_an = Res_CFA(detect_model= ckpt ['model'],device = device,data=source).to(device)
    ...
    optimizer = AdamW(params = params, lr = 1e-4,betas=(0.9, 0.999),weight_decay  =1e-4, amsgrad= True) 
    ...
    for epoch in range(start_epoch, epochs): 
        ...
        #         hp *= 255.0
        #         imgi = np.uint8(imgi.permute(1,2,0).detach().numpy()[:,:,[2,1,0]])
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default='runs/rescfa/train', help='save to project/name')
