...
def parse_opt():
    ...
    parser.add_argument('--weights_an', nargs='+', type=str, default=ROOT / '4kres_99.pt', help='model path(s)')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/res/detect', help='save results to project/name')
...
def run(weights=ROOT / 'yolov5x.pt',  # model.pt path(s)
    ...
    model = Res_CFA(ckpt=weights_an,detect=True,device=device)
    model.descriptor.eval()
    stride, names = model.detect_model.stride, model.detect_model.names
    names = ["car","truck","bus","person","fire","smoke","cone","div","suit","box","moto","hat","ma","bucket"]
    pt = True
    imgsz = check_img_size(imgsz, s=max(stride))  # check image size
    ...
    # for i in range(min(len(dataset_an),len(dataset))):
        ...
        # else:
        #     heatmaps_scores = np.stack((heatmaps_scores, heatmap_all), axis=0)
