...
def train(hyp, opt, device): 
    ...
    if pretrained:
        ...
        model_an = AN_Patch(weights=weights,device=device).to(device)
    ...
    optimizer = AdamW(model_an.parameters(), lr=1e-4, betas=(hyp['momentum'], 0.999)) 
    ...
    for epoch in range(start_epoch, epochs): 
        ...
        for i, (path, imgs,_, _, _) in pbar: 
            ...
            loss, _,tloss,floss = model_an(imgs)
            if loss == None:
                continue
            loss.backward()
            mem = f'{torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0:.3g}G'  # (GB)
            pbar.set_description(('%10s' * 2 + '%10.4g' * 3) % (
                    f'{epoch}/{epochs - 1}', mem, loss,tloss,floss))
            optimizer.step()
            if index % 10 == 0:
                writer.add_scalar('loss', loss.data.cpu(), index)
                writer.add_scalar('tloss', tloss.data.cpu(), index)
                writer.add_scalar('floss', floss.data.cpu(), index)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default='runs/simple_train', help='save to project/name')
