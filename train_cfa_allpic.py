...
def train(hyp, opt, device): 
    ...
    if pretrained:
        ...
        model_an = AN_CFA_ALL(weights=weights,device = device,data=source).to(device)
    ...
    train_loader, dataset = create_dataloader_cfa(source, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
    ...
    optimizer = AdamW(params = params, lr = 5e-4,betas=(0.9, 0.999),weight_decay  =5e-4, amsgrad= True) 
    ...
    for epoch in range(start_epoch, epochs): 
        ...
        for i, (imgs, targets, paths, _) in pbar: 
            ...
            if len(targets) == 0:
                continue
            loss , L_att , L_rep = model_an(imgs,targets)
            if loss ==None:
                continue
            loss.backward()
        ...
        dataset = LoadImages_resize(source_an, img_size=imgsz, stride=stride, auto=pt)
        dataset.__iter__()
        for j in range(20):
            ...
            path_an='_an_' +p_an 
            hp = (heatmap_an - heatmap_an.min()) / (heatmap_an.max() - heatmap_an.min())
            hp *= 255.0
            imgi = img_ins_an[0]* 255.0
            imgi = np.uint8(imgi.permute(1,2,0).numpy()[:,:,[2,1,0]])
            hp = cv2.applyColorMap(np.uint8(hp.permute(1,2,0)), cv2.COLORMAP_JET)
            visual_map = ((hp / 255.0) + (imgi / 255.0)) * 255.0
            visual_map = (visual_map / visual_map.max()) * 255.0
            newsave_dir = str(save_dir)+'/detect_'+str(epoch)
            newsave_dir = Path(newsave_dir )
            if not newsave_dir.exists():
                newsave_dir.mkdir(parents=True, exist_ok=True) 
            save_an_path = str(newsave_dir/path_an)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--project', default='runs/cfaall/train', help='save to project/name')
