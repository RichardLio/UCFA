...
def save_allhm_one(xyxys, im, file=Path('im.jpg'), gain=1.02, pad=10, square=False, BGR=False, save=True):
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    xyxy = xywh2xyxy(b).long()
    clip_coords(xyxy, im.shape)
...
def save_one_hm(xyxy, im,  hm, BGR=False, ):
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    xyxy = xywh2xyxy(b).long()
    clip_coords(xyxy, im.shape)
    xywh = xyxy2xywh(xyxy)
    im[int(xyxy[0, 1]):int(xyxy[0, 3]), int(xyxy[0, 0]):int(xyxy[0, 2]), ::(1 if BGR else -1)] += hm
    return im

def save_hm_mask(xyxy, im ):
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    xyxy = xywh2xyxy(b).long()
    clip_coords(xyxy, im.shape)
